...
class SAXBatteryCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def async_write_modbus_register(
        ...
        """Write value to Modbus register or update SAX item state."""
        try:
            # Handle SAX items differently - they don't use Modbus
            if isinstance(modbus_item, SAXItem):
                self._sax_item_states[modbus_item.name] = value
                await self.async_request_refresh()
                return True

            # Rest of Modbus logic...
            # Also fix the battery update check:
            battery = self.sax_data.batteries.get(self.battery_id)
            if battery is None:
                self._raise_battery_not_found_error()
                return False  # This will never be reached due to exception

            await battery.async_update()
            # Prevent frequent writes to same register
            last_write = self._last_write_time.get(modbus_item.address, 0)
            if time.time() - last_write < 1.0:
                await asyncio.sleep(1.0 - (time.time() - last_write))

            # Convert value according to item configuration
            converted_value = self._convert_value_for_writing(modbus_item, value)

            # Write to Modbus
            success = await self.modbus_api.write_holding_register(
                modbus_item.address, converted_value, self.battery_id
            )

            if success:
                self._last_write_time[modbus_item.address] = time.time()
                # Schedule immediate refresh for critical parameters
            ...
            _LOGGER.error(
                "Failed to write register %s for battery %s: %s",
                modbus_item.address,
                self.battery_id,
                err,
            )
    ...
    def _convert_value_for_writing(
        ...
        item_format = getattr(modbus_item, "format", None) or getattr(
    ...
    def _raise_battery_not_found_error(self) -> None:
    ...
    async def _read_smart_meter_item(self, item: ModbusItem) -> None:
        ...
        try:
            if hasattr(self.modbus_api, "read_holding_registers"):
