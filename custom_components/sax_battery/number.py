    ...
    def __init__(
        ...
        if hasattr(modbus_item, "description") and modbus_item.description:
            self._attr_native_min_value = getattr(
                modbus_item.description, "native_min_value", 0
            )
            self._attr_native_max_value = getattr(
                modbus_item.description, "native_max_value", 100
            )
            self._attr_native_step = getattr(modbus_item.description, "native_step", 1)
        else:
