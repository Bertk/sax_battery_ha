...
async def async_setup_entry(hass: HomeAssistant, entry: SAXBatteryConfigEntry) -> bool:
    ...
    try:
        ...
        # Auto-enable power limit entities if CONF_LIMIT_POWER is True
...
#     # SAX_MAX_DISCHARGE entity ID = "number.sax_bms_max_discharge"
#     # SAX_MAX_CHARGE entity ID    = "number.sax_bms_max_charge"
#     write_only_items = {
...
async def _enable_power_limit_entities(
    hass: HomeAssistant,
    entry: ConfigEntry,
    sax_data: SAXBatteryData,
) -> None:
    """Enable SAX_MAX_DISCHARGE and SAX_MAX_CHARGE entities when CONF_LIMIT_POWER is True.
    ...
    Security:
        OWASP A05: Validates entity existence before enabling

    Performance:
        Efficient registry lookups, batch operations
    """
    ...
    for item in power_limit_items:
        try:
            # Generate unique_id for cluster entity (battery_id=None)
            ...
            if not entity_id:
                _LOGGER.debug(
                    "Entity %s not yet in registry (will be created by platform setup)",
                    item.name,
                )
                continue
            ...
            if entity_entry and entity_entry.disabled_by is not None:
                ...
                enabled_count += 1
                _LOGGER.info(
                    "Enabled power limit entity %s (entity_id=%s) because CONF_LIMIT_POWER=True",
                    item.name,
                    entity_id,
                )
...
async def _get_battery_configurations(entry: ConfigEntry) -> dict[str, dict[str, Any]]:
    ...
    if CONF_BATTERIES in entry.data:
        ...
        master_battery = entry.data.get(CONF_MASTER_BATTERY, "bess_a")
...
async def _setup_battery_coordinator(
    ...
    Args:
        ...
        entry: Config entry
        sax_data: SAX Battery data manager
        battery_id: Battery identifier (bess_a, bess_b, bess_c)
