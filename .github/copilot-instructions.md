...
- new pymodbus version uses 'device_id' instead of 'slave'
...
- [Coordinated, single API poll for data for all entities (blueprint pattern)](https://developers.home-assistant.io/docs/integration_fetching_data/#coordinated-single-api-poll-for-data-for-all-entities)
...
**Always use `get_unique_id_for_item()` SAXBatteryData function** for entity unique ID generation. Never hardcode unique IDs.


```python
    # signature SAXBatteryData.get_unique_id_for_item
...
6. **Ensure https://developers.home-assistant.io/docs/development_testing/#running-a-limited-test-suite is followed**
...
### Implementation Recognition Rule (Critical)

**Before proposing ANY changes to existing code:**

1. **Mandatory Analysis Phase**
   - Read and understand the CURRENT implementation completely
   - Document existing function signatures, variable names, and class structures
   - Identify all dependencies and call sites
   - Verify actual behavior versus assumptions

2. **Breaking Changes Detection**
   - **Breaking Change Definition**: Any modification that requires updates to callers, tests, or dependent code:
     - Function/method signature changes (parameters, return types)
     - Variable/attribute renaming
     - Class constructor changes
     - Public API modifications

3. **Announcement Protocol**
   - **BEFORE implementing breaking changes**: Explicitly announce:
     - What will break and why
     - Impact scope (which files/tests need updates)
     - Justification for breaking existing API
   - Ask for confirmation before proceeding with breaking changes
   - Propose breaking changes as SEPARATE tasks from feature additions

4. **Preservation Requirements**
   - **When adding features**: Preserve existing functionality and signatures
   - **When fixing bugs**: Minimize scope of changes to affected code paths
   - **When optimizing**: Maintain existing interfaces and behavior
   - **Test compatibility**: Changes must not break existing unit tests unless tests themselves are incorrect

5. **Verification Checklist**
   - [ ] Current implementation read and understood
   - [ ] Function signatures documented (parameters, return types, exceptions)
   - [ ] Variable names and their purposes identified
   - [ ] Dependencies and call sites mapped
   - [ ] Breaking vs non-breaking changes classified
   - [ ] Test impact assessed

**Examples:**

❌ **BAD: Proposing breaking changes without analysis**
```python
# Without reading current code, propose:
async def read_holding_registers(self, item: ModbusItem, count: int = 1):
    # Changed parameter order - BREAKS ALL CALLERS
```
✅ **GOOD: Analyzing before proposing**
# Step 1: Read current implementation
# Current signature: async def read_holding_registers(self, count: int = 1, modbus_item: ModbusItem | None = None)
# Called by: coordinator._poll_device_batch(device, items)
#
# Step 2: Propose non-breaking enhancement
```python
async def read_holding_registers(self, count: int = 1, modbus_item: ModbusItem | None = None):
    # Add connection check WITHOUT changing signature
    if not self.is_connected():
        await self.connect()
    # Existing implementation continues...
```
❌ **BAD: Renaming without justification**
```python
# Rename _error_history to _errors without analysis
self._errors = deque(maxlen=100)  # BREAKS all references
```
✅ GOOD: Preserving existing names
```python
# Keep existing variable name, add new functionality
self._error_history = deque(maxlen=100)
# Add new method without breaking existing code
```

Rationale:

- Prevents repeated rework from misunderstanding current implementation
- Reduces test breakage and regression risk
- Maintains codebase stability during feature additions
- Respects existing architecture decisions
- Explicit breaking change announcement enables informed decisions
