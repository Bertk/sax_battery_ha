...
class TestSAXBatteryCoordinator:
    ...
    async def test_update_battery_not_found(
        ...
        with pytest.raises(UpdateFailed, match="Battery battery_a not found"):
    ...
    async def test_write_modbus_register_success(
        ...
        success = await coordinator.async_write_modbus_register(mock_modbus_item, 50.0)

        assert success
        mock_modbus_api.write_holding_register.assert_called_once_with(
            100,
            500,
            "battery_a",  # 50.0 * 10 (divider)
        )
    ...
    async def test_write_modbus_register_failure(
        ...
        mock_modbus_api.write_holding_register.return_value = False
    ...
    async def test_write_throttling(
        ...
        # First write
        await coordinator.async_write_modbus_register(mock_modbus_item, 50.0)
        ...
        with (
            ...
            await coordinator.async_write_modbus_register(mock_modbus_item, 60.0)
