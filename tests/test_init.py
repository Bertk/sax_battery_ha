...
class TestGetBatteryConfigurations:
    ...
    async def test_nested_configuration_format(self) -> None:
        ...
        assert len(result) == 2
        assert "bess_a" in result
        assert "bess_b" in result
        assert result["bess_a"][CONF_BATTERY_IS_MASTER] is True
        assert result["bess_b"][CONF_BATTERY_IS_MASTER] is False
...
class TestValidateBatteryConfig:
    ...
    def test_valid_configuration(self) -> None:
        ...
        assert _validate_battery_config("bess_a", config) is True
    ...
    def test_missing_required_keys(self) -> None:
        ...
        assert _validate_battery_config("bess_a", config) is False
    ...
    async def test_coordinator_setup_success(
        ...
        with (
            ...
            coordinator = await _setup_battery_coordinator(
                hass, entry, sax_data, "bess_a", battery_config
            )
            assert coordinator is not None
            assert coordinator.battery_id == "bess_a"
...
class TestValidationFunctions:
    ...
    def test_validate_battery_config_valid(self) -> None:
        ...
        assert _validate_battery_config("bess_a", battery_config) is True
    ...
    def test_validate_battery_config_missing_host(self) -> None:
        ...
        assert _validate_battery_config("bess_a", battery_config) is False
